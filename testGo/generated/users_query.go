...
func (uq *UsersQuery) QueryPets() *PetsQuery {
	query := (&PetsClient{config: uq.config}).Query()
...
func (uq *UsersQuery) WithPets(opts ...func(*PetsQuery)) *UsersQuery {
	query := (&PetsClient{config: uq.config}).Query()
...
func (uq *UsersQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Users, error) {
	...
	if query := uq.withPets; query != nil {
		if err := uq.loadPets(ctx, query, nodes,
			func(n *Users) { n.Edges.Pets = []*Pets{} },
			func(n *Users, e *Pets) { n.Edges.Pets = append(n.Edges.Pets, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (uq *UsersQuery) loadPets(ctx context.Context, query *PetsQuery, nodes []*Users, init func(*Users), assign func(*Users, *Pets)) error {
	...
	query.withFKs = true
	query.Where(predicate.Pets(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(users.PetsColumn), fks...))
	}))
	...
	for _, n := range neighbors {
		fk := n.users_pets
		if fk == nil {
			return fmt.Errorf(`foreign-key "users_pets" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "users_pets" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
